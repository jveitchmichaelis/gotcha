...
bool ALSC::doMatching(Point2f ptStartL, Point2f ptStartR, CTiePt& tp, const float* pfAffStart){
    ...
    for (int j = 0; j < nMaxIter; j++) {
        ...
        if (dist > fDisplacementThr)
            bNeed2Stop = true;
        if(fabs(ptStartL.y - ptUpdatedR.y) >= 3)
            bNeed2Stop = true;
        if (abs(emS(1)) > fAffineThr || isnan(emS(1)))
            bNeed2Stop = true;
        if (abs(emS(2)) > fAffineThr || isnan(emS(2)))
            bNeed2Stop = true;
        if (abs(emS(4)) > fAffineThr || isnan(emS(4)))
            bNeed2Stop = true;
        if (abs(emS(5)) > fAffineThr || isnan(emS(5)))
            bNeed2Stop = true;
        if (bNeed2Stop){
            break;
        }
    ...
    if (isnan(dEigenVal)) bNeed2Stop = true;
...
//        tp.m_fSimVal = dEigenVal/m_paramALSC.m_fEigThr; // normlise score, the smaller is the better
    ...
    //qDebug() << timer.nsecsElapsed() / 1000000000.0 << "ms";
